...
import { useDeleteRequestRewriteRule } from '../data/rules';

export function RulesDeleteDialog() {
  const { t } = useTranslation();
  const { open, setOpen, currentRow, resetRowSelection } = useRequestRewriteRules();
  const deleteMutation = useDeleteRequestRewriteRule();
  ...
  return (
    <AlertDialog open={open === 'delete'} onOpenChange={(isOpen) => !isOpen && setOpen(null)}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{t('requestRewriteRules.dialogs.delete.title')}</AlertDialogTitle>
          <AlertDialogDescription>
            {t('requestRewriteRules.dialogs.delete.description', { name: currentRow?.name })}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
