...
import { RequestRewriteRule, RequestRewriteRuleConnection } from '../data/schema';

interface RulesTableProps {
  columns: ColumnDef<RequestRewriteRule>[];
  data: RequestRewriteRule[];
  loading?: boolean;
  pageInfo?: RequestRewriteRuleConnection['pageInfo'];
  pageSize: number;
  totalCount?: number;
  nameFilter: string;
...
export function RulesTable({
  ...
  const { setSelectedRules, setResetRowSelection, setOpen } = useRequestRewriteRules();
  ...
  useEffect(() => {
    setSelectedRules(filteredSelectedRows.map((row) => row.original as RequestRewriteRule));
    ...
    <div className='flex flex-1 flex-col overflow-hidden'>
      <div className='mb-4 flex items-center justify-between'>
        <div className='flex flex-1 items-center space-x-2'>
          <Input
            placeholder={t('requestRewriteRules.filters.filterByName')}
